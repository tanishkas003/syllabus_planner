...
def heuristic_split_sections(text: str):
    ...
    cur = []
    for line in lines:
        if re.match(r'^(Module|Week|Unit|Chapter)\b', line, re.I) or (line.isupper() and len(line.split())<6):
            if cur:
                sections.append("\n".join(cur))
            cur = [line]
        else:
...
def extract_structured_syllabus(text: str):
    ...
    for sec in sections:
        ...
        lo = []
        m = re.search(r'(learning outcomes|learning objectives|outcomes)\s*[:\-]\s*(.*)', sec, re.I)
        if m:
